...
const CartProvider = ({ children }) => {
  ...
  const cartItems = cart
    .map((product) => {
      const matchedProduct = products.find((p) => p?.id === product?.productId); // Use `find` instead of `filter`
      ...
      if (isExist) {
        ...
        return prev.map((c) =>
          c.productId === product.id
            ? Number(qty) > 1 ? { ...c, quantity: c.quantity + Number(qty)} : { ...c, quantity: c.quantity + 1}
            : c
        );
      } else {
